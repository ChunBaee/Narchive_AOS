...
) : PagingSource<Int, ResponseFeed>() {
    override fun getRefreshKey(state: PagingState<Int, ResponseFeed>): Int? {
        return state.anchorPosition?.let { anchorPosition ->
            state.closestPageToPosition(anchorPosition)?.nextKey?.minus(1)
        }
    }

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ResponseFeed> {
        return try {
